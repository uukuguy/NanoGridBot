...
import { api, checkAuthStatus, changePassword as adapterChangePassword, updateProfile as adapterUpdateProfile, fetchAppearance } from '../api/adapter';
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  changePassword: async (currentPassword: string, newPassword: string) => {
    const data = await adapterChangePassword(currentPassword, newPassword);
    set({ user: data.user });
  },

  updateProfile: async (payload) => {
    const data = await adapterUpdateProfile(payload);
    set({ user: data.user });
  },
