...
## Phase 19: TUI 组件重构 (本会话新增)

**状态**: 🔄 进行中

### 背景

之前的 TUI 输入框实现存在多个复杂 bug：
- 折行后光标位置计算错误
- 超长输入不自动扩行
- 消息显示折行不一致
...
| 组件 | 评估 |
|------|------|
| tui-chat | ❌ 功能基础，不支持特殊消息类型 |
| tui-textarea | ✅ 功能完整，支持 Emacs 快捷键、撤销重做 |
| ratatui List | ✅ 消息显示（保持现有） |
| tui-markdown | ✅ 代码块渲染 |

### 最终方案

```
输入框: tui-textarea (最成熟，完整功能)
消息列表: ratatui List (保持现有)
代码块: tui-markdown
```
...
- ✅ 添加 tui-textarea 依赖
- ✅ 更新设计文档 `docs/plans/2026-02-20-ngb-tui-refactor-design.md`

### 待完成任务

| 任务 | 内容 |
|------|------|
| 1 | 用 tui-textarea 替换自定义输入组件 |
| 2 | 添加 tui-markdown 依赖 |
| 3 | 代码块使用 tui-markdown 渲染 |
| 4 | 实现 Ctrl+R 历史搜索面板 |
| 5 | 添加确认对话框支持 |
